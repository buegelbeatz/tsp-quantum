...
set -euo pipefail
# Purpose:
#   Execute the artifacts data 2 specification workflow for this layer.
# Security:
#   Reads and writes only within the repository. No sensitive data processed.

# Prompt entrypoint: /artifacts-data-2-specification
...
if [[ -z "$VENV_PATH" || ! -x "$VENV_PATH/bin/python" ]]; then
  echo "[artifacts-data-2-specification] ERROR: could not resolve synced layer venv python"
  exit 1
fi

export PYTHONPATH="$SCRIPT_DIR${PYTHONPATH:+:$PYTHONPATH}"
"$VENV_PATH/bin/python" "$SCRIPT_DIR/artifacts_flow.py" --repo-root "$REPO_ROOT" data-to-specification "$@"
